  ...
  describe "ソート機能" do
    ...
    context "終了期限でソートを押した場合" do
      it "終了期限が一番長いタスクが一番上に表示される" do
        FactoryBot.create(:task, title: "2日後のタスク", expired_at: Time.now + 2.days)
        FactoryBot.create(:task, title: "1日後のタスク", expired_at: Time.now + 1.days)
        click_on "終了期限でソート"
        sleep 2
    ...
    context "優先度高い順を押した場合" do
      it "優先度が一番高いタスクが一番上に表示される" do
        FactoryBot.create(:task, title: "優先度高いよ", priority: "高")
        FactoryBot.create(:task, title: "優先度低いよ", priority: "低")
        click_on "優先度高い順"
        sleep 2
        first_task = first(".task_row")
